...
# Get the backend root directory (2 levels up from processor.py: modules/speech_feedback/)
BACKEND_DIR = Path(__file__).parent.parent.parent

# Check multiple possible model locations
MODEL_PATHS = [
    os.path.join(BACKEND_DIR, "voice_feedback", "models", "sinhala_asr"),
    os.path.join(BACKEND_DIR, "models", "sinhala_asr"),
    os.path.join(BACKEND_DIR, "backend", "api", "models", "sinhala_asr"),
]
...
if MODEL_PATH is None:
    print(f"ERROR: Sinhala ASR model folder not found. Searched:")
    for p in MODEL_PATHS:
        print(f"  - {p}")
    print(f"Current working directory: {os.getcwd()}")
    raise FileNotFoundError(f"Sinhala ASR model folder not found in any expected location")
...
def get_word_timestamps(audio_path):
    ...
    try:
        ...
        with open("debug_voice.txt", "a", encoding="utf-8") as f:
            f.write(f"EXCEPTION IN get_word_timestamps:\n{traceback.format_exc()}\n")
