...
def migrate_metadata(clear_existing=False):
    ...
    # Check both standard and backup locations
    metadata_file = None
    ...
    with open(metadata_file, 'r', encoding='utf-8') as csvfile:
        ...
        if '|' in first_line:
            # Pipe-delimited format (backup format)
    ...
    if docs_to_insert:
        ...
        logger.info(f"✓ Migrated {len(result.inserted_ids)} metadata documents")
...
def migrate_timings():
    ...
    if isinstance(word_timings, list):
        if word_timings:
            ...
            logger.info(f"✓ Migrated {len(result.inserted_ids)} word timing documents")
        else:
            logger.warning("⚠ Timings JSON is empty")
    else:
        result = db.word_timings.insert_one(word_timings)
        logger.info(f"✓ Migrated word timings as single document")
...
def migrate_audio(clear_existing=True):
    ...
    logger.info(f"✓ Successfully migrated {success_count}/{len(audio_files)} audio files")
...
Examples:
  ...
  python migrate_to_mongodb.py --verify       # Check current state
    ...
    try:
        ...
        logger.error(f"\n✗ Operation failed: {str(e)}", exc_info=True)
        return False
